...
def negative_assert_symbol(first_name):
    ...
    # Проверяется, что  в теле ответа атрибут code — 400
...
# Тест 3. Негативная проверка. Количество символов меньше допустимого (1)
# Параметр fisrtName состоит из 1 символа
def test_create_user_1_letter_in_first_name_get_400_response():
    negative_assert_symbol('а')

# Тест 4. Негативная проверка. Количество символов больше допустимого (16)
# Параметр fisrtName состоит из 16 символов
def test_create_user_16_letter_in_first_name_get_400_response():
    negative_assert_symbol('Аааааааааааааааа')

# Тест 5. Успешное созание пользователя
# Параметр fisrtName состоит из английских букв
def test_create_user_english_letter_in_first_name_get_success_response():
    positive_assert("QWErty")

# Тест 6. Успешное созание пользователя
# # Параметр fisrtName состоит из русских букв
def test_create_user_russian_letter_in_first_name_get_success_response():
    positive_assert("Мария")

# Тест 7. Негативная проверка
# Параметр fisrtName состоит из пробелов
def test_create_user_has_space_in_first_name_get_error_response():
    negative_assert_symbol ("Человек и Ко")

# Тест 8. Негативная проверка
# Параметр fisrtName содержит запрещенные символы
...
# Функция для негативной проверки тестов 10 и 11
def negative_assert_no_first_name(user_body):
    ...
    # Проверяется, что  в теле ответа атрибут code — 400
    assert user_response.json()["code"] == 400
    #Проверяет, что в теле ответа атрибут message такой:
...
# Функция для негативной проверки: Передан другой тип параметра firstName: число
def negative_assert_number_first_name(first_name):
    # В переменную user_body сохраняется обновленное тело запроса
    user_body = get_user_body(first_name)
    # В переменную user_response сохраняется результат запроса на создание пользователя:
    user_response = sender_stand_request.post_new_user(user_body)
...
def test_create_user_number_type_first_name_get_error_response():
    negative_assert_number_first_name(12)
